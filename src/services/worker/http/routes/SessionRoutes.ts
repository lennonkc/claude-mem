...
import { OpenRouterProvider, isOpenRouterSelected, isOpenRouterAvailable, isCustomOpenAICompatibleSelected, isCustomOpenAICompatibleAvailable } from '../../OpenRouterProvider.js';
...
export class SessionRoutes extends BaseRouteHandler {
  ...
  private getActiveAgent(): ClaudeProvider | GeminiProvider | OpenRouterProvider {
    if (isCustomOpenAICompatibleSelected()) {
      if (isCustomOpenAICompatibleAvailable()) {
        logger.debug('SESSION', 'Using Custom (OpenAI-compatible) agent');
  ...
  private getSelectedProvider(): 'claude' | 'gemini' | 'openrouter' | 'custom-openai-compatible' {
    if (isCustomOpenAICompatibleSelected() && isCustomOpenAICompatibleAvailable()) {
  ...
  private async startGeneratorWithProvider(
    ...
    provider: 'claude' | 'gemini' | 'openrouter' | 'custom-openai-compatible',
    source: string
  ): Promise<void> {
    if (!session) return;
    ...
    const agent = (provider === 'openrouter' || provider === 'custom-openai-compatible') ? this.openRouterAgent : (provider === 'gemini' ? this.geminiAgent : this.sdkAgent);
    const agentName = provider === 'custom-openai-compatible' ? 'Custom (OpenAI-compatible)' : (provider === 'openrouter' ? 'OpenRouter' : (provider === 'gemini' ? 'Gemini' : 'Claude SDK'));
